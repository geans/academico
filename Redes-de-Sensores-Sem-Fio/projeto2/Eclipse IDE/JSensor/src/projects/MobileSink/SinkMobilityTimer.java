...
public class SinkMobilityTimer extends MobilityModel {
	private Hashtable<Integer, Integer> nodes;
	
	public SinkMobilityTimer(){
		this.nodes = new Hashtable<>();
	}
	
	public SinkMobilityTimer(Hashtable<Integer, Integer> nodes){
		this.nodes = nodes;
	}
	
    @Override
    public MobilityModel clone() {
        return new SinkMobilityTimer(this.nodes);
    }
    ...
    public Position nextMoviment(Node n){
    	...
    	if(nodes.containsKey(n.getID())){
    		...
    		//if is stopped.
    		if(direction == 8){
    			//probability of remain stopped = 0 //0.8
    			if(n.getRandom().nextDouble() < 0.)
